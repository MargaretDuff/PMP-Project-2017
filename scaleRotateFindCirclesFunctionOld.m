function [ numberofeggs ] = scaleRotateFindCirclesFunctionOld(img)
% % Read file
...
minIndex = sortIndex(1:floor(8*n/100));
...
for Ang=0:5:175
    ...
    %     For 0.25 1mm images
    [centres, radii, ~]=imfindcircles(new, [11,33], 'ObjectPolarity', 'bright', 'Sensitivity', 0.82,'EdgeThreshold', 0);
    ...
    if ~isempty(radii)
        ...
        I3gray=rgb2gray(I3);
        I4=I3;
        %Check intesnity and radius to eliminate obvious non-eggs
        for k=1:length(radii)
            ...
            if intensity1>50 && intensity1<95
                if intensity2>70 && intensity2<95
                    foundbefore=0;
                    %Check if this egg has been found before
                    for u=-25:25
                        for v=-25:25
                    ...
                    I4=insertShape(I4, 'circle',[centres(k,:)  radii(k)], 'Color','white', 'LineWidth', 5);
